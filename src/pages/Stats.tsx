...
const Stats = () => {
    ...
    const getMedalColor = (rank: number) => {
        switch (rank) {
            case 1: return 'bg-gradient-to-b from-yellow-400 to-yellow-300';
            case 2: return 'bg-gradient-to-b from-gray-300 to-gray-200';
            case 3: return 'bg-gradient-to-b from-amber-600 to-amber-500';
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-indigo-50 to-white pb-20">
            ...
            <div className="relative mx-6 mt-8 p-8 bg-gradient-to-r from-purple-600 to-indigo-700 rounded-3xl shadow-2xl text-center overflow-hidden">
                {/* Decorative Icon */}
                <div className="absolute top-4 right-4 text-white text-4xl opacity-10 animate-bounce-slow">🚀</div>

                {/* Content */}
                <div className="z-10 relative">
                    <div className="inline-flex items-center justify-center w-16 h-16 bg-white rounded-full shadow-lg mb-4 mx-auto animate-fade-in">
                    ...
                    <p className="text-indigo-100 text-sm sm:text-base">We're working hard to bring this feature to life. Stay tuned!</p>
                    <span className="inline-block mt-4 px-4 py-2 text-xs font-semibold bg-white/20 rounded-full text-white tracking-wide uppercase">
                        Under Development
                    </span>
                </div>
            </div>
            ...
            {/* <div className="bg-white mx-6 mt-6 p-4 rounded-xl shadow-sm">
                <div className="grid grid-cols-2 gap-4">
                    <div>
                        <label className="block text-xs text-gray-500 font-medium mb-1">Academic Year</label>
                        <select
                            value={selectedYear}
            ...
            {/* {!loading && Object.keys(topPerformers).length > 0 && (
                <div className="mx-6 mt-6 space-y-6">
                    {Object.entries(topPerformers).map(([className, performers]) => (
                        <div key={className} className="bg-white rounded-xl shadow-sm overflow-hidden">
                            <div className="bg-indigo-50 px-4 py-3 border-b border-indigo-100">
                                <h3 className="font-medium text-indigo-800">Grade {className}</h3>
                            </div>
                                    ...
                                    <div key={student.student_id} className="p-4 hover:bg-gray-50 transition-colors">
                                        <div className="flex items-center">
                                            <div className="relative">
                                                ...
                                                />
                                                <div className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-white ${getMedalColor(student.rank)}`}>
                                            ...
                                            <div className="text-right">
                                                <div className={`text-lg font-bold ${student.average_score >= 90 ? 'text-yellow-600' :
                                                        student.average_score >= 80 ? 'text-indigo-600' :
                                                            'text-gray-700'
                                                    }`}>
            ...
            {/* {!loading && Object.keys(topPerformers).length === 0 && !error && (
                <div className="mx-6 mt-6 bg-white p-6 rounded-xl shadow-sm text-center">
                    <svg className="w-10 h-10 mx-auto text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
                    </svg>
                    <p className="mt-3 text-gray-500">No top performers data available</p>
                    <p className="text-sm text-gray-400 mt-1">Please select a different year or term</p>
                </div>
            )} */}
        </div>
    );
